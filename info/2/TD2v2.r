...
prop.table(table(classe$Indice, classe$Genre), 1)

# Profil colonne

prop.table(table(classe$Indice, classe$Genre), 2)
...
isff = na.omit(ISF_2014)
length(isff$codecom)

#Nb communes uniques :
length(unique(ISF_2014$codecom))
length(unique(isff$codecom))
...
head(tapply(ISF_2014v2$patrimoine_moy,ISF_2014v2$nouvelle_region,mean))

head(tapply(ISF_2014v2$patrimoine_moy,ISF_2014v2$region,mean))
